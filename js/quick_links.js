...
jQuery(function($){
	...
	quickDataFns = {
		...
		message_list: {
			title: '购物车',
			content: '<div class="ibar_plugin_content"><div class="ibar_cart_group ibar_cart_product"><div class="ibar_cart_group_header"><span class="ibar_cart_group_title"></span><a href="#">我的购物车</a></div></div><div class="cart_handler"><div class="cart_handler_header"><span class="cart_handler_left">共<span class="cart_price">1</span>件商品</span><span class="cart_handler_right"></span><span class="rmb">￥</span></div><a href="shopping-bag01.html" class="cart_go_btn" target="_blank">去购物车结算</a></div></div>',
			init:$.noop
		},
	...
	$("body").on("click",".shopping",function(){
		var item = {};
		var pro = $(this).parent().siblings(".introCont");
		item.imgsrc = $(this).siblings("img").attr("src");
		item.pname = pro.find("a").text();
		item.price = pro.find(".price").text();
		item.cont = 1;
		var f = $.inArray(item.pname,shoppingcardataname);
		f == -1 ? (shoppingcardata.push(item),shoppingcardataname.push(item.pname)) : shoppingcardata[f].cont++;
		var c = parseInt($(".quick_links span.cart_num").text());
		$(".quick_links span.cart_num").text(++c);
	})

	function getCont(obj,data)
	{
		var str = "";
		$.each(data,function(){
			str += '<li class="cart_item""><div class="cart_item_pic"><a href="#"><img src="'+ this.imgsrc +'" /></a></div><div class="cart_item_desc"><a href="#" class="cart_item_name">'+ this.pname +'</a><div class="cart_item_sku"><span>尺码：38码（精工限量版）</span></div><div class="cart_item_price"><span class="cart_price">'+ this.price +'</span></span><span class="mul">x</span><span class="cart_cont">'+ this.cont +'</div></div><span class="del"></span></li>'
		})
		obj.html('<ul>' + str + '</ul>');
	}

	function getACP(obj,data)
	{
		var ac = 0 ,ap = 0;
		$.each(data,function(){
			ac += this.cont;
			ap += this.price.replace("￥","") * this.cont;
		})
		$(".cart_handler_header .cart_price").text(ac);
		$(".cart_handler_header .cart_handler_right").text(ap);
		$(".quick_links span.cart_num").text(ac);

	}

	$(".quick_link_mian").on("click",".del",function(){
		var sproname = $(this).parent().find("cart_item_name").text();
		var f = $.inArray(sproname,shoppingcardataname);
		shoppingcardataname.splice(f,1);
		shoppingcardata.splice(f,1);
		getCont($(".ibar_cart_group"),shoppingcardata);
		getACP($(".ibar_cart_group"),shoppingcardata);
